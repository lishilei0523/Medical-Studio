...
namespace MedicalSharp.Controls.Commands
{
    ...
    public class DrawCurveCommand : DrawShapeCommand
    {
        ...
        public DrawCurveCommand(bool closed)
        {
            this._closed = closed;
        }
        ...
        #region 绘制开始委托 —— Action<CurveVisual3D> DrawStart
        /// <summary>
        /// 绘制开始委托
        /// </summary>
        public Action<CurveVisual3D> DrawStart { get; set; }
        #endregion

        #region 绘制结束委托 —— Action<CurveVisual3D> DrawEnd
        /// <summary>
        /// 绘制结束委托
        /// </summary>
        public Action<CurveVisual3D> DrawEnd { get; set; }
        #endregion

        #region 绘制已取消委托 —— Action<CurveVisual3D> DrawCancelled
        /// <summary>
        /// 绘制已取消委托
        /// </summary>
        public Action<CurveVisual3D> DrawCancelled { get; set; }
        #endregion
            ...
            if (eventArgs.Properties.IsLeftButtonPressed && viewport is BasicViewport basicViewport)
            {
                ...
                if (mousePos3D.HasValue)
                {
                    ...
                    if (this._curve == null)
                    {
                        ...
                        this.DrawStart?.Invoke(this._curve);
                    }
                    else
                    {
                        //先移除预览点（如果存在）
        ...
        private void CompleteDrawing(OpenTKViewport viewport)
        {
            ...
            //绘制结束
            this._isDrawing = false;
            this.DrawEnd?.Invoke(this._curve);
        ...
        private void CancelDrawing(OpenTKViewport viewport)
        {
            if (this._curve != null)
            {
                this.DrawCancelled?.Invoke(this._curve);
            }
