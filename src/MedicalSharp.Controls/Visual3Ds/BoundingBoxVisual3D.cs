    ...
    public class BoundingBoxVisual3D : ShapeVisual3D, IPureVisual3D, ITranslatable3D, IRotatable, IResizable3D, IHasSurfaceArea, IHasVolume, ICutVolume, IAnalyseVolume3D
    {
        ...
        public async Task<StatisticResult> ApplyAnalyseVolume(VolumeData volumeData, byte? markValue)
        {
            ...
            foreach (Vector3 corner in this.Bounds.Corners)
            {
                ...
                worldMin = Vector3.ComponentMin(worldMin, worldPos);
                worldMax = Vector3.ComponentMax(worldMax, worldPos);
            }

            //转换到体素坐标
            Vector3i minVoxelPos = worldMin.ToVoxelPosition(volumeData.Metadata);
            Vector3i maxVoxelPos = worldMax.ToVoxelPosition(volumeData.Metadata);
