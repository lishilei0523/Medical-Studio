...
namespace MedicalSharp.Controls.Visual3Ds
{
    ...
    public class RectangleVisual3D : ShapeVisual3D, IVisual2DIn3D, ITranslatable3D, IRotatable, IResizable2D, IResizable3D, IHasPerimeter, IHasSurfaceArea, ICutVolume, IAnalyseVolume2D
    {
        ...
        public bool TryGetResizeAxis(Ray localRay, out ResizeContext3D resizeContext)
        {
            ...
            [
                (face: HitFace.Right,  point: center + uAxis * halfW, normal:  uAxis),
                (face: HitFace.Left,   point: center - uAxis * halfW, normal: -uAxis),
                (face: HitFace.Top,    point: center + vAxis * halfH, normal:  vAxis),
                (face: HitFace.Bottom, point: center - vAxis * halfH, normal: -vAxis)
            ];
        ...
        public void ApplyResize(ResizeContext3D resizeContext, Vector3 localHitPoint)
        {
            ...
            float dotU = Math.Abs(Vector3.Dot(resizeContext.Axis, this.UAxis.ToVector3()));
            float dotV = Math.Abs(Vector3.Dot(resizeContext.Axis, this.VAxis.ToVector3()));
            if (dotU > 0.99f)
            {
        ...
        public void ApplyCutVolume(VolumeData volumeData, Texture3D markTexture, CutMode cutMode, byte markValue)
        {
            ...
            Vector3 vAxis = this.VAxis.ToVector3();
            Matrix4 localToWorld = this.Transform.Matrix;
            volumeData.ApplyRectangleCut(markTexture, this.Width, this.Height, center, normal, uAxis, vAxis, localToWorld, cutMode, markValue);
        }
        #endregion
        ...
        public StatisticResult ApplyAnalyseVolume(MPRViewport viewport, byte? markValue)
        {
            ...
            StatisticResult result = viewport.VolumeData.ApplyRectangleAnalyse(screenPointA, screenPointB, screenPointC, screenPointD, viewportWidth, viewportHeight, layerPixels, markValue);
