...
namespace MedicalSharp.Controls.Visual3Ds
{
    ...
    public class CylinderVisual3D : ShapeVisual3D, IPureVisual3D, ITranslatable3D, IRotatable, IResizable3D, IHasSurfaceArea, IHasVolume, ICutVolume, IAnalyseVolume3D
    {
        ...
        public float CalculateSurfaceArea(VolumeMetadata metadata)
        {
            ...
            //沿Z轴方向的偏移向量（半高）
            Vector3 localHalfHeightOffset = new Vector3(0, 0, this.Height / 2);

            //中心点、边界点的世界坐标
            Vector3 worldCenter = Vector3.TransformPosition(localCenter, localToWorld);
            Vector3 worldRadiusBoundary = Vector3.TransformPosition(localCenter + localRadiusOffset, localToWorld);
            Vector3 worldTopBoundary = Vector3.TransformPosition(localCenter + localHalfHeightOffset, localToWorld);

            //转换为毫米坐标
            Vector3 mmCenter = worldCenter.ToMillimeterPosition(metadata);
            Vector3 mmRadiusBoundary = worldRadiusBoundary.ToMillimeterPosition(metadata);
            Vector3 mmTopBoundary = worldTopBoundary.ToMillimeterPosition(metadata);

            //半径和半高
            float mmRadius = Vector3.Distance(mmCenter, mmRadiusBoundary);
            float mmHalfHeight = Vector3.Distance(mmCenter, mmTopBoundary);
            float mmHeight = mmHalfHeight * 2;

            //圆柱体表面积 = 2πr² + 2πrh
            float area = 2 * MathF.PI * mmRadius * mmRadius + 2 * MathF.PI * mmRadius * mmHeight;
        ...
        public float CalculateVolume(VolumeMetadata metadata)
        {
            ...
            //圆柱体体积 = πr²h
            float volume = MathF.PI * mmRadius * mmRadius * mmHeight;
        ...
        public async Task<StatisticResult> ApplyAnalyseVolume(VolumeData volumeData, byte? markValue)
        {
            ...
            //转换到体素坐标
            Vector3i minVoxelPos = minWorld.ToVoxelPosition(volumeData.Metadata);
            Vector3i maxVoxelPos = maxWorld.ToVoxelPosition(volumeData.Metadata);
