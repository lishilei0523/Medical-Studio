...
namespace MedicalSharp.Client.ViewModels.VolumeContext
{
    ...
    public class MprAxialViewModel : ScreenBase
    {
        ...
        /// 依赖注入构造器
        /// </summary>
        public MprAxialViewModel(IWindowManager windowManager, IEventAggregator eventAggregator)
        {
            this._windowManager = windowManager;
            this._eventAggregator = eventAggregator;
            this._eventAggregator.SubscribeOnUIThread(this);

            const string title = "Axial";
            MPRCamera camera = new MPRCamera();
            MPRInputManager inputManager = new MPRInputManager(camera);
            this.MprViewModel = new MprViewModel(windowManager, eventAggregator, title, camera, MPRPlaneType.Axial, inputManager);
